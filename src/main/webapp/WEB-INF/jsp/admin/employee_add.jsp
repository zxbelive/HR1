...
<body class="gray-bg">
					...
					<div class="ibox-content">
						<form method="post" class="form-horizontal" id="commentForm" action="<%=path %>/employee/add.do">
							...
							<div class="form-group">
								...
								<div class="col-sm-7">
									<select class="form-control m-b" name="departmentNumber" id="dep" onchange="x()" size="1" required>
							...
							<%----%>
										...
										<%--%>--%>
									<%--</select>--%>
								<select class="form-control m-b" name="positionNumber" id="pos" size="1" required >
									<option>--请选择职称--</option>
								</select>
								</div>
							</div>
							...
							<div class="form-group">
								<div class="col-sm-4 col-sm-offset-8">
									...
									<a class="btn btn-white" type="reset" href="/employee/listPage.do?pageNo=1">取&nbsp;&nbsp;消</a>
								</div>
							</div>
						</form>
					</div>
	...
	<script>
		function x() {
			$("#pos").html("<option disabled=disabled>--请选择职称--</option>")
			var v=$("#dep").val();
			<%
			for (Department department : dList) {
			%>
			if (v==<%=department.getDepartmentNumber()%>){
				if (<%=department.getName().equals("人事部")%>){
					<%
					List<Position> pList = (List<Position>) request.getAttribute("pList");
					for (Position p : pList) {
					%>
					if (<%=p.getLevel().equals("人事")%>) {
						$("#pos").append("<option value='<%=p.getPositionNumber()%>'><%=p.getName()%></option>")
					}
					<%}%>
				}else {
					<%for (Position position : pList) {
					%>
					if (<%=!position.getLevel().equals("人事")%>) {
						$("#pos").append("<option value='<%=position.getPositionNumber()%>'><%=position.getName()%></option>")
					}
					<%}
					%>
				}
